...
local function createSprintingPed(session)
    ...
    TaskGoStraightToCoord(ped, destination.x, destination.y, destination.z, 2.0, -1, destination.w, 3.0)

    CreateThread(function()
        repeat
            Wait(500)
        until #(GetEntityCoords(ped) - destination.xyz) < 2.0 or IsEntityDead(ped)
...
function Mode.startMode(session)
    ...
    CreateThread(function()
        while started do
            ...
            if #(GetEntityCoords(playerPed) - startPosition.xyz) > 5.0 then
